...
function ensureModelPresent(allModels: Model<any>[], model: Model<any>): void {
	if (!allModels.some((candidate) => candidate.provider === model.provider && candidate.id === model.id)) {
		allModels.push(model);
	}
}

function ensureModelsPresent(allModels: Model<any>[], models: Model<any>[]): void {
	for (const model of models) {
		ensureModelPresent(allModels, model);
	}
}

const CURATED_FALLBACK_MODELS: Model<any>[] = [
	{
...
const STABLE_OPENROUTER_MODELS: Model<"openai-completions">[] = [
	{
...
async function generateModels() {
	...
	ensureModelsPresent(allModels, CURATED_FALLBACK_MODELS);
	...
	// Keep a few important OpenRouter catalog IDs stable even if upstream metadata changes.
	// These models have been observed tool-capable and are useful for vision workflows.
	ensureModelsPresent(allModels, STABLE_OPENROUTER_MODELS);
